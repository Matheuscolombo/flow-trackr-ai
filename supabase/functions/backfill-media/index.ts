...
Deno.serve(async (req) => {
  ...
  // Find messages with CDN or invalid storage URLs
  ...
  // Filter to only messages that need re-downloading (CDN URLs or broken storage URLs)
  const needsDownload = (messages || []).filter(m => 
    m.media_url?.includes("mmg.whatsapp.net") || 
    m.media_url?.includes("mmg-fna.whatsapp.net")
  );
